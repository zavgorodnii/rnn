...
class MultilayerLSTM(object):
    ...
    def __init__(self, num_hidden_neurons=4, num_lstm_layers=2, eta=0.1):
    ...
    def train(self, training_data):
        ...
        lstm = tf.nn.rnn_cell.BasicLSTMCell(
            self.num_hidden_neurons, state_is_tuple=True)
        stacked_lstm = tf.nn.rnn_cell.MultiRNNCell(
            [lstm] * self.num_lstm_layers, state_is_tuple=True)
        # N.B.: LSTM's zero state is a little bit more complex that just a
        # single tensor.
        self._initial_hidden = stacked_lstm.zero_state(batch_size, tf.float32)
        ...
        self.hidden, _final_hidden = tf.nn.rnn(
            stacked_lstm, rnn_inputs, initial_state=self._initial_hidden)
